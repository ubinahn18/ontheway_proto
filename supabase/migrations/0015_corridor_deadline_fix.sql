-- items_within_corridor predates the pickup/delivery-deadline columns
-- (0013) and never returned them, so every search result's
-- delivery_deadline came back undefined client-side — silently breaking
-- the "latest pickup by" deadline-feasibility check added on top of it.
